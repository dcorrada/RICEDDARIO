...
our $mdp_file = $ENV{HOME} . '/GOLEM_RAWDATA/tools/20ns_NPT.mdp';
...
SEARCH_FILES: {
    ...
    foreach my $record (keys %targets) {
        ...
        my $obj = SPARTA::lib::FileIO->new('filename'=>$top);
        my $content = $obj->read();
...
PARSE_TOPOL: {
    ...
    foreach my $record (keys %targets) {
        ...
        my $ions = '';
        $/ = ';'; # uso ';' come separatore di campo per leggere i file topol.top
        my $top_obj = SPARTA::lib::FileIO->new('filename'=> $workdir . $top_file);
...
#                 print $molecules;
        ...
        # leggo il templato
        my $template_obj = SPARTA::lib::FileIO->new('filename' => $mdp_file);
        ...
        SWITCH: {
            ...
            /chlorine/ and do {
                ...
                my $new_mdp = SPARTA::lib::FileIO->new('filename' => $workdir . $mdp_filename, 'filedata' => \@mdp_patched);
                $new_mdp->write();
                push(@{$targets{$record}->{'files'}}, $workdir . $mdp_filename);
                print "done";
                last SWITCH;
