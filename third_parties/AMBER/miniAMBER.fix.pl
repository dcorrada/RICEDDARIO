...
This script is  a variant of <miniAMBER.pl> aimed to perform the miniAMBER 
workflow over already optimized structures that doesn't need strong minimization 
and/or relevant relaxing stages.

The main steps of this MODIFIED workflow are:
    ...
    B) TOPOLOGY, launching tleap to obtain parameters and coordinate files of 
       receptor, ligand and complex systems.
    
    C) MINIMIZATION, 250 steps steepest descent + 750 coniugated gradient 
    
    D) HEATING, in NVT ensemble to 300K (10ps, dt 2fs)
    
    E) EQUILIBRATION, in NPT ensemble at 300K (10ps, dt 2fs)
    
PAY ATTENTION: in the steps C), D) and E) position restraints (120 kcal/molA) 
will be applied to the whole structure of both receptor and ligand.
...
MINIMIZATION: {
    ...
    $sander_script = <<END
Minimisation: solute restraints (500 kcal/molA)
&cntrl
 imin = 1,
 maxcyc = 1000,
 ncyc = 250,
...
END
    ;
    open(SANDRO, ">$workdir/min.in");
    print SANDRO $sander_script;
    close SANDRO;
    
    # lancio la minimizzazione
    $cmd = "cd $workdir; $bins{'sander'} -O -i min.in -p solv.rec.prmtop -c solv.rec.inpcrd -ref solv.rec.inpcrd -r min.rec.rst7 -o min.rec.mdout";
    ...
    unless ($ligprefix eq 'null') { 
        ...
        $cmd = "cd $workdir; $bins{'sander'} -O -i min.in -p solv.com.prmtop -c solv.com.inpcrd -ref solv.com.inpcrd -r min.com.rst7 -o min.com.mdout";
        printf("%s MINIMIZATION complex started...\n", clock());
        push @thr, threads->new(\&launch_thr, $cmd, "MINIMIZATION complex");
        
        # minimizzazione del ligando
        $cmd = "cd $workdir; $ligsander -O -i min.in -p solv.lig.prmtop -c solv.lig.inpcrd -ref solv.lig.inpcrd -r min.lig.rst7 -o min.lig.mdout";
...
END
    ...
    $cmd = "cd $workdir; $bins{'sander'} -O -i eq.nvt.in -p solv.rec.prmtop -c min.rec.rst7 -ref min.rec.rst7 -r eq.nvt.rec.rst7 -o eq.nvt.rec.mdout -x eq.nvt.rec.mdcrd -e eq.nvt.rec.mden";
    printf("%s HEATING receptor started...\n", clock());
    push @thr, threads->new(\&launch_thr, $cmd, "HEATING receptor");
    
    unless ($ligprefix eq 'null') {
        # lancio la simulazione del ligando
        $cmd = "cd $workdir; $ligsander -O -i eq.nvt.in -p solv.lig.prmtop -c min.lig.rst7 -ref min.lig.rst7 -r eq.nvt.lig.rst7 -o eq.nvt.lig.mdout -x eq.nvt.lig.mdcrd -e eq.nvt.lig.mden";
        printf("%s HEATING ligand started...\n", clock());
        push @thr, threads->new(\&launch_thr, $cmd, "HEATING ligand");
        
        # lancio la simulazione del complesso
        $cmd = "cd $workdir; $bins{'sander'} -O -i eq.nvt.in -p solv.com.prmtop -c min.com.rst7 -ref min.com.rst7 -r eq.nvt.com.rst7 -o eq.nvt.com.mdout -x eq.nvt.com.mdcrd -e eq.nvt.com.mden";
...
EQUILIBRATION: {
    ...
    $sander_script = <<END
Equilibrating the system in NPT ensemble (10 ps)
&cntrl
 imin = 0,
 ntx = 5,
 irest= 1,
...
END
    ...
    # lancio la simulazione del recettore
    $cmd = "cd $workdir; $bins{'sander'} -O -i eq.npt.in -p solv.rec.prmtop -c eq.nvt.rec.rst7  -ref eq.nvt.rec.rst7 -r eq.npt.rec.rst7 -o eq.npt.rec.mdout -x eq.npt.rec.mdcrd -e eq.npt.rec.mden";
    printf("%s EQUILIBRATION receptor started...\n", clock());
    push @thr, threads->new(\&launch_thr, $cmd, "EQUILIBRATION receptor");
    
    unless ($ligprefix eq 'null') {
        # lancio la simulazione del ligando
        $cmd = "cd $workdir; $ligsander -O -i eq.npt.in -p solv.lig.prmtop -c eq.nvt.lig.rst7 -ref eq.nvt.lig.rst7 -r eq.npt.lig.rst7 -o eq.npt.lig.mdout -x eq.npt.lig.mdcrd -e eq.npt.lig.mden";
        printf("%s EQUILIBRATION ligand started...\n", clock());
        push @thr, threads->new(\&launch_thr, $cmd, "EQUILIBRATION ligand");
        
        # lancio la simulazione del complesso
        $cmd = "cd $workdir; $bins{'sander'} -O -i eq.npt.in -p solv.com.prmtop -c eq.nvt.com.rst7 -ref eq.nvt.com.rst7 -r eq.npt.com.rst7 -o eq.npt.com.mdout -x eq.npt.com.mdcrd -e eq.npt.com.mden";
