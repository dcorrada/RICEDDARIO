...
## Again, credit to user 'jschiwal' for the next two lines
     ...
     trjconv -s $file -f $file -n protein_only.ndx -o traj_$file.xtc -t0 $num
...
trjcat -f *.pdb.xtc -o trajectory.xtc

## Convert the first frame to an input .gro file

editconf -f mdcrd_0.pdb -n protein_only.ndx -o input_t0.gro
